...
bool Dictionary::compile(const Param &param,
  ...
  for (size_t i = 0; i < dics.size(); ++i) {
    ...
    while (is->getline(line.get(), line.size())) {
      ...
      token->posid  = pid;
      token->wcost = cost;
      token->feature = static_cast<unsigned int>(offset);
      token->compound = 0;
  ...
  for (size_t i = 0; i < dic.size(); ++i) {
    if (i != 0 && prev != dic[i].first) {
      ...
      val.push_back(static_cast<Darts::DoubleArray::result_type>(bsize +(idx << 8)));
      bsize = 1;
      idx = i;
    } else {
      ++bsize;
  ...
  unsigned int lsize = static_cast<unsigned int>(matrix.left_size());
  unsigned int rsize = static_cast<unsigned int>(matrix.right_size());
  unsigned int dsize = static_cast<unsigned int>(da.unit_size() * da.size());
  unsigned int tsize = static_cast<unsigned int>(tbuf.size());
  unsigned int fsize = static_cast<unsigned int>(fbuf.size());
